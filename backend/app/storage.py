...
from .config import JOB_META_FILENAME, LOG_FILENAME, RESULTS_BASE_DIR
...
def job_dir(job_id: str) -> Path:
    return RESULTS_BASE_DIR / job_id
...
def iter_job_ids() -> Iterable[str]:
    if not RESULTS_BASE_DIR.exists():
        return []
    return [p.name for p in RESULTS_BASE_DIR.iterdir() if p.is_dir()]
